...
export default function ChatsList({ chats }: { chats?: Chat[] }) {
  ...
  return (
    <div
      className="max-w-3xl w-full px-2 sm:px-4 py-2 justify-items-start flex flex-col gap-2 max-h-60 sm:max-h-80 shadow-md rounded-md border border-gray-200 bg-white"
    >
        ...
        {[...chats].reverse().map((chat: Chat) => {
          return (
            ...
            >
              ...
              >
                <MessageCircleMore size={16} className="flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  {editingId === chat.id ? (
                    <input
                      autoFocus
                      className="text-xs sm:text-sm border rounded px-1 w-full"
                      value={editingTitle}
                    ...
                    <h2 className="text-xs sm:text-sm font-medium truncate">{chat.title}</h2>
                  )}
                </div>
              </button>
              <div className="flex items-center justify-center w-12 sm:w-16 h-8 flex-shrink-0 relative">
                <span className="absolute inset-0 flex items-center justify-center text-[9px] sm:text-xs text-gray-400 group-hover:opacity-0 transition-opacity whitespace-nowrap">
